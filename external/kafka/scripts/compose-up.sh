...
for SERVICE in $KAFKA_SERVICES; do
  ...
  until MSYS_NO_PATHCONV=1 docker exec "$SERVICE" /opt/kafka/bin/kafka-metadata-quorum.sh --bootstrap-server localhost:"$INT_PORT" describe --status >/dev/null 2>&1; do
    sleep 1
  done
done
...
if [ -n "$FIRST_NODE" ]; then
  ...
  MSYS_NO_PATHCONV=1 docker exec "$FIRST_NODE" /opt/kafka/bin/kafka-metadata-quorum.sh --bootstrap-server localhost:"$INT_PORT" describe --status
fi
